...
def remove_unnecessary_whitespace(css):
    ...
    css = _spaces1_re.sub(
        lambda match: match.group(0).replace(":", "___PSEUDOCLASSCOLON___"), css
    )
...
def normalize_rgb_colors_to_hex(css):
    ...
    while match:
        colors = tuple(map(lambda s: int(s.strip()), match.group(1).split(",")))
        css = css.replace(match.group(), f"#{colors[0]}{colors[1]}{colors[2]}")
        match = regex.search(css)
    return css
...
def handle_imports(css):
    imports = set()
    regex = re.compile(r"@import url\(([^)]*)\);?")

    for match in regex.finditer(css):
        import_statement = css[match.start() : match.end()]
